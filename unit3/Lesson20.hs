...
makeTSCompare f = g
  where
    g (i, Nothing) (_, Nothing) = (i, Nothing)
    g (_, Nothing) (i, v) = (i, v)
    g (i, v) (_, Nothing) = (i, v)
    g (i, Just vi) (j, Just vj)
      | f vi vj == vi = (i, Just vi)
      | otherwise = (j, Just vj)
...
compareTS f (TS times values)
  ...
  | otherwise = Just best
  where
    best = foldr (makeTSCompare f) (0, Nothing) $ zip times values
...
diffTS (TS ts vs) = TS ts (Nothing : diffValues)
  where
    diffValues = zipWith diffPair (tail vs) vs

meanMaybe :: Real a => [Maybe a] -> Maybe Double
meanMaybe vals
  | any isNothing vals = Nothing
  | otherwise = Just (mean $ map fromJust vals)
